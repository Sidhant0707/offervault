...
export default function DashboardPage() {
  ...
  // 3. Initial Data Fetching (Session + Offers)
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      if (!data.user) {
        router.replace("/login");
      } else {
        // Parse email for dynamic display name
        ...
        // Fetch user's actual submissions from the database
  ...
  const handleDeleteSubmission = async (offerId: string) => {
    ...
    try {
      ...
      // Optimistic UI Update (remove from screen instantly)
      setMySubmissions((prev) => prev.filter((offer) => offer.id !== offerId));
    } catch (error) {
  ...
  if (!authChecked) {
    return (
      <main className="min-h-screen flex items-center justify-center">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Verifying identity...</p>
      </main>
    );
  }
        ...
        <div className="grid lg:grid-cols-12 gap-12 mt-8">
          ...
          <aside className="lg:col-span-3 space-y-8 lg:sticky lg:top-32 h-fit">
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-6 truncate">{userData.email}</p>
              ...
              <div className="pt-6 border-t border-white/10 space-y-4">
                <div>
                  ...
                  <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: 0 }} animate={{ width: `${userData.trustScore}%` }}
                      transition={{ duration: 1, delay: 0.5 }} className="h-full bg-white rounded-full"
                    />
                  </div>
                </div>
            ...
            <motion.nav initial="hidden" animate="visible" variants={fadeUp} className="space-y-2">
              {[
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
                    activeTab === item.id ? "bg-white text-black" : "bg-transparent text-slate-500 hover:bg-[#0a0a0a] hover:text-white"
                  }`}
                >
          ...
          <div className="lg:col-span-9 space-y-8">
            <motion.div initial="hidden" animate="visible" variants={fadeUp} className="p-6 rounded-2xl border border-white/10 bg-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed max-w-xl">Upload a redacted offer letter to verify your entry. Verified data helps thousands of peers negotiate better.</p>
                </div>
              </div>
            ...
            <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="min-h-[500px]">
              ...
              {activeTab === "submissions" && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between border-b border-white/10 pb-6">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{mySubmissions.length} Records</span>
                  </div>
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    {mySubmissions.map((record) => (
                      <DataRecordCard 
                        key={record.id} 
                        college={record.company} 
                        branch={record.role}
                        year={record.created_at ? new Date(record.created_at).getFullYear().toString() : ''}
                        company={record.company}
                        ctc={String(record.salary)}
                        status={record.status || "Pending Verification"}
                        category="My Submission"
                        onDelete={() => handleDeleteSubmission(record.id)} 
                      />
                    ))}
              ...
              {activeTab === "saved" && (
                <div className="space-y-6">
                  ...
                  <div className="p-16 border border-white/5 rounded-2xl bg-[#050505] text-center">
                        ...
                        <p className="text-xs text-slate-500 max-w-sm mx-auto">Explore the leaderboard or database to bookmark institutions and track their placement trends.</p>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-8 border border-white/5 rounded-2xl bg-[#050505] space-y-8">
                    ...
                    <div>
                      ...
                      <button 
                        type="button" onClick={handlePurgeAccount} disabled={isDeleting}
